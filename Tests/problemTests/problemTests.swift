...
final class problemTests: XCTestCase {
    ...
    func testProblem7() throws {
        ...
        XCTAssertEqual(p.run(n: 10001), 104743)
    }
    ...
    func testProblem10() throws {
        ...
        XCTAssertEqual(p.run(max: 2000000), 142913828922)
    }
    ...
    func testProblem12() throws {
        ...
        XCTAssertEqual(p.run(n: 500), 76576500)
    }
    ...
    func testProblem23() throws {
        ...
        XCTAssertEqual(p.run(28123), 4179871)
    }
    ...
    func testProblem25() throws {
        ...
        XCTAssertEqual(p.run(1000), 4782)
    }
    ...
    func testProblem27() throws {
        ...
        //XCTAssertEqual(p.run(min: -999, max: 999), -59231)
    }
    ...
    func testProblem39() throws {
        ...
        XCTAssertEqual(p.run(max: 1000), 840)
    }
    ...
    func testProblem51() throws {
        ...
        //XCTAssertEqual(p.run(expectedSize: 8, primedigits: 6), 121313)
    }
    ...
    func testProblem76() throws {
        ...
        XCTAssertEqual(p.run(d: 100), 190569291)
    }
    func testProblem77() throws {
        let p = problem77()
        XCTAssertEqual(p.run(d: 5), 10)
        XCTAssertEqual(p.run(d: 5_000), 71)
    }
    func testProblem78() throws {
        let p = problem78()
        XCTAssertEqual(p.run(d: 10), 9)
        XCTAssertEqual(p.run(d: 100), 74)
        XCTAssertEqual(p.run(d: 1000000), 55374)
    }
    func testProblem79() throws {
        let p = problem79()
        XCTAssertEqual(p.run(), 73162890)
    }
    func testProblem80() throws {
        let p = problem80()
        XCTAssertEqual(p.run(num: 2, precision: 100), 475)
        XCTAssertEqual(p.run(num: 100, precision: 100), 40886)
    }
    func testProblem81() throws {
        let p = problem81()
        let grid = [
            [131, 673, 234, 103, 18],
            [201, 96, 342, 965, 150],
        ...
        XCTAssertEqual(p.run(grid: grid), 2427)
        XCTAssertEqual(p.run(), 427337)
    }
    func testProblem82() throws {
        let p = problem82()
        let grid = [
            [131, 673, 234, 103, 18],
            [201, 96, 342, 965, 150],
    ...
    func testProblem84() throws {
        let p = problem84()
        XCTAssertEqual(p.run(d: 6), "102400")
        XCTAssertEqual(p.run(d: 4), "101524")
    }
    func testProblem85() throws {
        let p = problem85()
        XCTAssertEqual(p.run(target: 18), 6)
        XCTAssertEqual(p.run(target: 2_000_000), 2772)
    }
    func testProblem86() throws {
        let p = problem86()
        XCTAssertEqual(p.run(N: 1974), 99)
        XCTAssertEqual(p.run(N: 2059), 100)
        XCTAssertEqual(p.run(N: 1_000_000), 1818)
    }
    func testProblem87() throws {
        let p = problem87()
        XCTAssertEqual(p.run(max: 50), 4)
        XCTAssertEqual(p.run(max: 50_000_000), 1097343)
    }
    func testProblem89() throws {
        let p = problem89()
        XCTAssertEqual(p.run(), 743)
    }
    func testProblem91() throws {
        let p = problem91()
        XCTAssertEqual(p.run(n: 2), 14)
        XCTAssertEqual(p.run(n: 50), 14234)
    }
    ...
    func testProblem95() throws {
        let p = problem95()
        XCTAssertEqual(p.run(max: 1_000), 220)
        //XCTAssertEqual(p.run(max: 1_000_000), 14316)
    }
    func testProblem96() throws {
        let p = problem96()
        XCTAssertEqual(p.run(from: 0, to: 0), 483)
        XCTAssertEqual(p.run(from: 0, to: 49), 24702)
    }
    func testProblem97() throws {
        let p = problem97()
        XCTAssertEqual(p.run(), 8739992577)
    }
    func testProblem99() throws {
        let p = problem99()
        XCTAssertEqual(p.run(), 709)
    }
    func testProblem100() throws {
        let p = problem100()
        XCTAssertEqual(p.run(start: 20), 15)
        XCTAssertEqual(p.run(start: 25), 85)
        XCTAssertEqual(p.run(start: 1_000_000_000_000), 756872327473)
    }
}
